...
import com.example.surfacecamera.adapter.EffectAdapter;
import com.example.surfacecamera.adapter.MenuAdapter;
import com.example.surfacecamera.adapter.SenseAdapter;
import com.example.surfacecamera.base.BaseActivity;
import com.example.surfacecamera.util.AppConstant;
import com.example.surfacecamera.util.cameravideo.CameraHelper;
import com.example.surfacecamera.util.cameravideo.ICamera2;
import com.example.surfacecamera.util.cameravideo.IVideoControl;
import com.example.surfacecamera.util.cameravideo.VideoPlayer;
import com.example.surfacecamera.view.AutoFitTextureView;
import com.example.surfacecamera.view.AutoLocateHorizontalView;
import com.example.surfacecamera.view.AwbSeekBar;


import static com.example.surfacecamera.util.AppConstant.SHOW_AE;
import static com.example.surfacecamera.util.AppConstant.SHOW_AWB;

@RequiresApi(api = Build.VERSION_CODES.M)

public class CameraShowActivity extends BaseActivity implements IVideoControl.PlaySeekTimeListener,
        IVideoControl.PlayStateListener, ICamera2.TakePhotoListener, SensorEventListener,
        ICamera2.CameraReady, AutoLocateHorizontalView.OnSelectedPositionChangedListener,
     ...
     * 焦点框
     */
    @BindView(R.id.video_focus)
    ImageView videoFocus;
    /**
     * zoom 缩小
     */
    ...
    private Runnable mImageFoucesRunnable = new Runnable() {
        @Override
        public void run() {
            videoFocus.setVisibility(View.GONE);
        }
    };
    /**
     * 3s后隐藏的runnable
    ...
    protected int getLayoutId() {
        return R.layout.activity_camera_show;
    }

    @Override
    protected void initView() {
        videoSwitchCamera.setOnClickListener(this);
        videoSwitchFlash.setOnClickListener(this);
        videoRecord.setOnClickListener(this);
        videoDelete.setOnClickListener(this);
        videoSave.setOnClickListener(this);
        // 将底部布局 依次添加到 列表中
        mLayoutList.clear();
...
//            Log.e(TAG, "onSensorChanged: x: " + x +"   y: "+y +"  z : "+z);
    ...
    public void onStartListener(int width, int height) {
        ...
        videoMinePlay.setImageResource(R.drawable.ic_pause);
    ...
    public void onCompletionListener() {
        hasPlaying = false;
        videoMinePlay.setImageResource(R.drawable.ic_play);
    ...
    public void onTakePhotoFinish(final File file, int photoRotation, int width, int height) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Glide.with(CameraShowActivity.this).load(file).into(videoPhoto);
//                videoPhoto.setImageURI(cameraHelper.getUriFromFile(CameraShowActivity.this, file));
    ...
    public void recordVideoOrTakePhoto() {
        ...
        if (NOW_MODE == AppConstant.VIDEO_RECORD_MODE) {
            if (!hasRecording) {
                ...
                if (hasRecording) {
                    videoRecord.setImageResource(R.drawable.ic_recording);
                    hindSwitchCamera();
                    recordCountDown();
                    hindMenu();
...
//    @OnClick(R.id.video_save)
    ...
    private void removeImageFoucesRunnable() {
        videoFocus.removeCallbacks(mImageFoucesRunnable);
    }
    ...
    private void imageFoucesDelayedHind() {
        videoFocus.postDelayed(mImageFoucesRunnable, 500);
    }
    ...
    private void moveFouces(int x, int y) {
        videoFocus.setVisibility(View.VISIBLE);
        RelativeLayout.LayoutParams layoutParams
                = (RelativeLayout.LayoutParams) videoFocus.getLayoutParams();
        videoFocus.setLayoutParams(layoutParams);
        mFoucesAnimation.setDuration(500);
        mFoucesAnimation.setRepeatCount(0);
        mFoucesAnimation.setOldMargin(x, y);
        videoFocus.startAnimation(mFoucesAnimation);
    ...
    private class FoucesAnimation extends Animation {
        ...
        private int width = cameraHelper.dip2px(CameraShowActivity.this, 150);
        private int W = cameraHelper.dip2px(CameraShowActivity.this, 65);
